...
test('8. in_progress -> completed', () => assert.strictEqual(ok(evaluateTaskTransition({ task: base({ status: 'in_progress' }), action: TASK_ACTION.COMPLETE, context: { completionSource: COMPLETION_SOURCE.QUIZ_SYNC } })).status, 'completed'));
