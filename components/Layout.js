...
export default function Layout({ children, title = 'SSC GK SCORE BOOSTER — AI Prep', hideAuth = false }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#FF8C00] to-[#FF6B35] font-sans text-gray-900 flex flex-col">
      <Head>
        <title>{title}</title>
          ...
          {status === 'loading' ? (
            ...
            <div className="flex items-center gap-2 bg-white shadow-sm border border-gray-100 rounded-full pl-1 pr-3 py-1 scale-90 origin-right">
              ...
              <div className="flex flex-col">
                <span className="text-[10px] font-bold text-gray-700 leading-tight truncate max-w-[80px]">
                  {session.user.name}
                </span>
              </div>
            </div>
            ...
            <button
              ...
              className="bg-white shadow-sm border border-gray-100 rounded-full px-4 py-1.5 text-[10px] font-bold text-gray-600 hover:bg-gray-50 transition active:scale-95 flex items-center gap-2 scale-90 origin-right"
            >
