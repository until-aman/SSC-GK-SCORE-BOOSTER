export default function PodiumCard({ performer, rank }) {
  ...
  const podiumColor = isFirst ? 'bg-[#7E61FF]' : isSecond ? 'bg-[#6B4DFF]' : 'bg-[#5A3CFF]';
  ...
  return (
    <div className={`flex flex-col items-center flex-1 justify-end animate-pop-in relative z-${10 - rank}`} style={{ animationDelay: `${rank * 0.1}s` }}>
      ...
      <div className="relative mb-4">
        <div className={`${avatarSize} rounded-full border-4 border-white/20 shadow-[0_10px_25px_rgba(0,0,0,0.2)] overflow-hidden bg-white/10 p-0.5 transform transition hover:scale-105`}>
          <img 
      ...
      <div className="text-center mb-5 w-full px-1">
        <p className="font-black text-white text-xs uppercase tracking-wider mb-2 drop-shadow-md truncate">{performer.name.split(' ')[0]}</p>
        ...
        <div className="mt-4 flex flex-col items-center">
          ...
          <div className={`w-14 h-14 rounded-full border-4 border-white/30 flex items-center justify-center shadow-xl ${medalColor} group-hover:scale-110 transition-transform`}>
            <div className="w-10 h-10 rounded-full border-2 border-white/50 flex items-center justify-center">
              <span className="text-white font-black text-2xl drop-shadow-sm">{rank}</span>
            </div>
          </div>
        </div>
      </div>
