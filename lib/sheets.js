...
/**
 * Append a V2 score row (14 columns) to the Scores sheet.
 * Col N (index 13) = streakMilestoneBonus
 */
async function appendScoreV2({ timestamp, email, name, correctAnswers, incorrectAnswers, skipped, totalQuestions, rawScore, subject, topic, sessionId, xpEarned, isDailyChallenge = 'FALSE', streakMilestoneBonus = 0 }) {
  const sheets = await getSheetsClient();
  await sheets.spreadsheets.values.append({
    spreadsheetId: process.env.GOOGLE_SHEET_ID,
    range: 'Scores!A:N',
    ...
    requestBody: {
      values: [[
        ...
        subject, topic, sessionId, xpEarned, isDailyChallenge, streakMilestoneBonus,
      ]],
    },
  });
}
