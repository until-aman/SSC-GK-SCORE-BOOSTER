...
async function appendScoreV2({
  ...
  setRowValueByHeader(row, headerIndex, ['xpEarned'], xpEarned);
  ...
  setRowValueByHeader(row, headerIndex, ['totalXP'], totalXP);
...
async function getDailyChallengeEntry(dateStr) {
  return cachedRead(`dailyChallenge:${dateStr}`, async () => {
    ...
    return {
      ...
      xpReward: Number(oldRows[0]?.[4]) || 50,
      status: 'Active',
      layout: 'legacy',
    };
...
async function writeDailyChallengeEntry({ date, challengeId, questionIds, totalQuestions, xpReward, status = 'Active' }) {
  const sheets = await getSheetsClient();
  await sheets.spreadsheets.values.append({
    spreadsheetId: process.env.GOOGLE_SHEET_ID,
