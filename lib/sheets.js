...
const { instrumentSheetsClient } = require('./apiDiagnostics');
const { dedupeSheetsReads } = require('./server/sheetsReadDedup');
...
async function getSheetsClient() {
  const auth = getAuth();
  // Inner: instrumentSheetsClient (dev-only physical-op logging; no-op in prod).
  // Outer: dedupeSheetsReads (ALL envs) — collapses identical concurrent
  // values.get / values.batchGet to one physical read. Dedup must be outer so
  // the inner physical log fires exactly once (only for the read that actually
  // invokes the underlying method). Args/returns/errors unchanged.
  return dedupeSheetsReads(instrumentSheetsClient(google.sheets({ version: 'v4', auth })));
}
...
// ── SSC Mentor helpers — appended, do not modify above ───────────────────────

async function getMasterTopics(sheets) {
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: process.env.GOOGLE_SHEET_ID,
    range: 'MasterTopics!A2:J',
  });
  const rows = res.data.values || [];
  const headers = ['TopicId','SubjectId','SubjectName','TopicName','DisplayName',
                   'DisplayOrder','QuestionCount','SSCWeightage','IsActive','Notes'];
  return rows
    .map(row => {
      const obj = {};
...
async function getMasterSubjects(sheets) {
  let res;
  try {
    res = await sheets.spreadsheets.values.get({
      spreadsheetId: process.env.GOOGLE_SHEET_ID,
      range: "'Master Subjects'!A2:F",
    });
  } catch {
    res = await sheets.spreadsheets.values.get({
      spreadsheetId: process.env.GOOGLE_SHEET_ID,
      range: 'MasterSubjects!A2:F',
    });
  }
  const rows = res.data.values || [];
  const headers = ['SubjectId','SubjectName','DisplayOrder','Icon','IsActive','ShortName'];
  return rows
    .map(row => {
      const obj = {};
      headers.forEach((h, i) => { obj[h] = row[i] || ''; });
...
// Gap 1 + Gap 2 Fix: reads A2:M (13 columns), returns camelCase keys
async function getMentorProfile(sheets, email) {
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: process.env.GOOGLE_SHEET_ID,
    range: 'MentorProfile!A2:M',
  });
  const rows = res.data.values || [];
  const headers = [
  ...
  const found = rows.find(row => row[0] === email);
  if (!found) return null;
  const raw = {};
  headers.forEach((h, i) => { raw[h] = found[i] || ''; });

  // Return camelCase to match plan engine and API response (Gap 2 Fix)
  let subjectStatus = {};
  let topicsCompleted = {};
  let topicStrength = {};
  try { subjectStatus = JSON.parse(raw.SubjectStatusJSON || '{}'); } catch {}
  try { topicsCompleted = JSON.parse(raw.TopicsCompletedJSON || '{}'); } catch {}
  try { topicStrength = JSON.parse(raw.TopicStrengthJSON || '{}'); } catch {}

  return {
    email: raw.Email,
    examTarget: raw.ExamTarget,
    daysLeftRange: raw.DaysLeftRange,
    customDaysLeft: raw.CustomDaysLeft || null,
    dailyGKTime: raw.DailyGKTime,
    pace: raw.Pace,
    goals: raw.Goals ? raw.Goals.split('|') : [],
    subjectStatus,
    topicsCompleted,
    topicStrength,
...
// Gap 1 Fix: writes 13 columns including TopicStrengthJSON
async function upsertMentorProfile(sheets, email, profileData) {
  const checkRes = await sheets.spreadsheets.values.get({
    spreadsheetId: process.env.GOOGLE_SHEET_ID,
    range: 'MentorProfile!A2:A',
  });
  const rows = checkRes.data.values || [];
  const rowIndex = rows.findIndex(r => r[0] === email);
  const now = new Date().toISOString();

  // Preserve onboardingCompletedAt on first write
  const isNew = rowIndex === -1;
  const onboardingDate = isNew ? now : (profileData.onboardingCompletedAt || now);

  const rowValues = [
    email,
    profileData.examTarget || '',
    profileData.daysLeftRange || '',
    profileData.customDaysLeft || '',
    profileData.dailyGKTime || '',
    profileData.pace || '',
    Array.isArray(profileData.goals) ? profileData.goals.join('|') : (profileData.goals || ''),
    JSON.stringify(profileData.subjectStatus || {}),
    JSON.stringify(profileData.topicsCompleted || {}),
    onboardingDate,
    now,
    'v1',
    JSON.stringify(profileData.topicStrength || {}),   // Column M
  ];

  if (isNew) {
    await sheets.spreadsheets.values.append({
      spreadsheetId: process.env.GOOGLE_SHEET_ID,
      range: 'MentorProfile!A:M',
    ...
    await sheets.spreadsheets.values.update({
      spreadsheetId: process.env.GOOGLE_SHEET_ID,
      range: `MentorProfile!A${sheetRow}:M${sheetRow}`,
      valueInputOption: 'RAW',
      requestBody: { values: [rowValues] },
    });
  }
...
async function appendTaskFeedback(sheets, feedbackData) {
  const now = new Date().toISOString();
  const row = [
    now,
    feedbackData.email || '',
    ...
    feedbackData.topic || '',
    feedbackData.quizSessionId || '',
    feedbackData.feedbackChip || '',
    feedbackData.resultCategory || '',
    feedbackData.correctRate ?? 0,
    feedbackData.wrongRate ?? 0,
    feedbackData.skippedRate ?? 0,
    feedbackData.totalQuestions ?? 0,
    feedbackData.quizMode || '',
    feedbackData.sourceTaskId || '',
    feedbackData.sourcePage || '',
    feedbackData.mentorNextAction || '',
    feedbackData.mentorActionSavedAt || now,
  ];
  await sheets.spreadsheets.values.append({
    spreadsheetId: process.env.GOOGLE_SHEET_ID,
    range: 'TaskFeedback!A:P',
    valueInputOption: 'RAW',
    requestBody: { values: [row] },
  });
}

const MENTOR_CACHE_VERSION = 'v1';
const MENTOR_PROFILE_EXTRA_HEADERS = [
  'MentorPlanId',
  'ActiveDayNumber',
  'ProgressPercent',
  'LastPlanRefreshAt',
...
const STUDENT_TOPIC_STATE_HEADERS = [
  'Email','UserId','Subject','Topic','TheoryStatus','PracticeStatus','ConfidenceLevel',
  'QuestionsPracticedSelfReported','LastTheoryUpdatedAt','LastPracticeUpdatedAt',
  'LastConfidenceUpdatedAt','LastQuizAttemptAt','RecentAccuracy','RevisionGapDays',
  'MentorPriorityScore','StateVersion','UpdatedAt',
];
const MENTOR_PLAN_HEADERS = [
...
const MENTOR_TASK_HEADERS = [
  'TaskId','PlanId','Email','UserId','DayNumber','SequenceNumber','Type','Subject','Topic',
  'Title','MentorMessage','Reason','WhyThisText','PrimaryAction','SecondaryAction','Status',
  'IsRequiredForUnlock','DependsOnTaskIds','Source','SourceReference','SnoozeCount',
  'SnoozedUntil','CreatedAt','ActivatedAt','CompletedAt','UpdatedAt',
];
const MENTOR_TASK_LOG_HEADERS = [
  'LogId','TaskId','PlanId','Email','UserId','ActionType','ActionValue','CreatedAt',
  'SourcePage','QuizSessionId','Notes',
];

function mentorHeaderIndex(headers) {
...
async function readHeaderSheet(sheets, tabName) {
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: process.env.GOOGLE_SHEET_ID,
    range: `${tabName}!A:ZZ`,
  });
  ...
  return {
    headers,
    headerIndex: mentorHeaderIndex(headers),
    rows: values.slice(1),
  };
}

function buildHeaderRow(headers, valuesByHeader) {
...
async function appendMentorRows(sheets, tabName, rowsByHeader) {
  if (!rowsByHeader.length) return;
  const { headers } = await readHeaderSheet(sheets, tabName);
  if (!headers.length) throw new Error(`${tabName} header row is missing`);
  const values = rowsByHeader.map(row => buildHeaderRow(headers, row));
  await sheets.spreadsheets.values.append({
    spreadsheetId: process.env.GOOGLE_SHEET_ID,
    range: `${tabName}!A:ZZ`,
    valueInputOption: 'RAW',
...
async function updateMentorRow(sheets, tabName, sheetRowNumber, valuesByHeader) {
  const { headers, headerIndex, rows } = await readHeaderSheet(sheets, tabName);
  if (!headers.length) throw new Error(`${tabName} header row is missing`);
  const existing = rows[sheetRowNumber - 2] || [];
  const next = [...existing];
  ...
  await sheets.spreadsheets.values.update({
    ...
    valueInputOption: 'RAW',
    requestBody: { values: [buildHeaderRow(headers, Object.fromEntries(headers.map((h, i) => [h, next[i] || ''])))] },
  });
}
...
function normalizePlan(row) {
  ...
  return {
    ...
    email: row.Email,
    userId: row.UserId || '',
    version: row.Version || MENTOR_CACHE_VERSION,
    status: row.Status || 'active',
    examSnapshot: row.ExamSnapshot || '',
    daysLeftSnapshot: row.DaysLeftSnapshot || '',
    paceSnapshot: row.PaceSnapshot || '',
    activeDayNumber: Number(row.ActiveDayNumber || 1),
    progressPercent: Number(row.ProgressPercent || 0),
    lastGeneratedFromStateVersion: row.LastGeneratedFromStateVersion || '',
...
function normalizeMentorTask(row) {
  ...
  return {
    ...
    taskType: row.Reason === 'recent_mistakes' ? 'practice_task' : row.Type,
    subject: row.Subject || '',
    subjectName: row.Subject || '',
    topic: row.Topic || '',
    title: row.Title || '',
    displayName: row.Title || '',
    mentorMessage: row.MentorMessage || '',
    reason: row.Reason || '',
    whyThisText: row.WhyThisText || '',
    ctaLabel: row.PrimaryAction || '',
    completionAction: row.Reason === 'recent_mistakes' ? 'Task is done' : '',
    secondaryAction: row.SecondaryAction || 'Maybe later',
    status: row.Status || 'pending',
    isRequiredForUnlock: String(row.IsRequiredForUnlock || '').toUpperCase() === 'TRUE',
    dependsOnTaskIds: String(row.DependsOnTaskIds || '').split(',').map(v => v.trim()).filter(Boolean),
    source: row.Source || '',
    ...
    updatedAt: row.UpdatedAt || '',
    estimatedMinutes: row.Type === 'coverage_check' || row.Type === 'confidence_check' ? 3 : 15,
    questionCount: row.Type === 'practice_task' || row.Type === 'revision_task' || row.Type === 'mistake_recovery_task' ? 25 : null,
  };
}

async function getMentorProfileWithPlanState(sheets, email) {
  const profile = await getMentorProfile(sheets, email);
  if (!profile) return null;
  const { headers, headerIndex, rows } = await readHeaderSheet(sheets, 'MentorProfile');
  const row = rows.find(item => item[headerIndex.Email] === email) || [];
  MENTOR_PROFILE_EXTRA_HEADERS.forEach(header => {
    const index = headerIndex[header];
    if (typeof index !== 'number') return;
    profile[header.charAt(0).toLowerCase() + header.slice(1)] = row[index] || '';
  });
  profile.mentorCacheVersion = profile.mentorCacheVersion || MENTOR_CACHE_VERSION;
  return profile;
}

async function updateMentorProfilePlanState(sheets, email, values = {}) {
  const { headerIndex, rows } = await readHeaderSheet(sheets, 'MentorProfile');
  const rowIndex = rows.findIndex(row => row[headerIndex.Email] === email);
  if (rowIndex === -1) return;
  const allowed = {};
  MENTOR_PROFILE_EXTRA_HEADERS.forEach(header => {
    if (Object.prototype.hasOwnProperty.call(values, header)) allowed[header] = values[header];
  });
  if (!Object.keys(allowed).length) return;
  await updateMentorRow(sheets, 'MentorProfile', rowIndex + 2, allowed);
}

async function getStudentTopicState(sheets, email) {
  const { headers, rows, headerIndex } = await readHeaderSheet(sheets, 'StudentTopicState');
  return rows
    .filter(row => row[headerIndex.Email] === email)
    .map(row => rowObject(headers, row));
}

async function upsertStudentTopicState(sheets, email, state) {
  const { headers, rows, headerIndex } = await readHeaderSheet(sheets, 'StudentTopicState');
  const subject = state.Subject || state.subject || '';
  const topic = state.Topic || state.topic || '';
  const rowIndex = rows.findIndex(row => row[headerIndex.Email] === email && row[headerIndex.Subject] === subject && row[headerIndex.Topic] === topic);
  const existing = rowIndex === -1 ? {} : rowObject(headers, rows[rowIndex]);
  const now = new Date().toISOString();
  const row = {
    Email: email,
    UserId: state.UserId || state.userId || existing.UserId || '',
    Subject: subject,
    Topic: topic,
    TheoryStatus: state.TheoryStatus || state.theoryStatus || existing.TheoryStatus || 'unknown',
    PracticeStatus: state.PracticeStatus || state.practiceStatus || existing.PracticeStatus || 'unknown',
    ConfidenceLevel: state.ConfidenceLevel || state.confidenceLevel || existing.ConfidenceLevel || 'unknown',
    QuestionsPracticedSelfReported: state.QuestionsPracticedSelfReported || state.questionsPracticedSelfReported || existing.QuestionsPracticedSelfReported || '',
    LastTheoryUpdatedAt: state.LastTheoryUpdatedAt || state.lastTheoryUpdatedAt || existing.LastTheoryUpdatedAt || '',
    LastPracticeUpdatedAt: state.LastPracticeUpdatedAt || state.lastPracticeUpdatedAt || existing.LastPracticeUpdatedAt || '',
    LastConfidenceUpdatedAt: state.LastConfidenceUpdatedAt || state.lastConfidenceUpdatedAt || existing.LastConfidenceUpdatedAt || '',
    LastQuizAttemptAt: state.LastQuizAttemptAt || state.lastQuizAttemptAt || existing.LastQuizAttemptAt || '',
    RecentAccuracy: state.RecentAccuracy ?? state.recentAccuracy ?? existing.RecentAccuracy ?? '',
    RevisionGapDays: state.RevisionGapDays ?? state.revisionGapDays ?? existing.RevisionGapDays ?? '',
    MentorPriorityScore: state.MentorPriorityScore ?? state.mentorPriorityScore ?? existing.MentorPriorityScore ?? '',
    StateVersion: state.StateVersion || state.stateVersion || existing.StateVersion || MENTOR_CACHE_VERSION,
    UpdatedAt: now,
  };
  if (rowIndex === -1) await appendMentorRows(sheets, 'StudentTopicState', [row]);
  else await updateMentorRow(sheets, 'StudentTopicState', rowIndex + 2, row);
}

async function getActiveMentorPlan(sheets, email) {
  const { headers, rows, headerIndex } = await readHeaderSheet(sheets, 'MentorPlans');
  const activeRows = rows
    .map(row => rowObject(headers, row))
    .filter(row => row.Email === email && row.Status === 'active')
    .sort((a, b) => new Date(b.UpdatedAt || b.CreatedAt || 0) - new Date(a.UpdatedAt || a.CreatedAt || 0));
  const plan = normalizePlan(activeRows[0]);
  if (!plan) return null;
  const tasks = await getMentorTasksForPlan(sheets, email, plan.planId);
  return { ...plan, tasks };
}

async function getMentorTasksForPlan(sheets, email, planId) {
  const { headers, rows } = await readHeaderSheet(sheets, 'MentorTasks');
  return rows
    .map(row => rowObject(headers, row))
    .filter(row => row.Email === email && row.PlanId === planId)
    .map(normalizeMentorTask)
    .filter(Boolean)
    .sort((a, b) => a.dayNumber - b.dayNumber || a.sequenceNumber - b.sequenceNumber);
}

async function createMentorPlanSnapshot(sheets, email, profile, generatedPlan, userId = '') {
  const now = new Date().toISOString();
  const planId = generatedPlan.planId || `MP_${Date.now()}`;
  const existingPlans = await readHeaderSheet(sheets, 'MentorPlans').catch(() => null);
  if (existingPlans?.rows?.length) {
    const { rows, headerIndex } = existingPlans;
    await Promise.all(rows.map((row, index) => {
      if (row[headerIndex.Email] === email && String(row[headerIndex.Status] || '').toLowerCase() === 'active') {
        return updateMentorRow(sheets, 'MentorPlans', index + 2, {
          Status: 'invalid',
          UpdatedAt: now,
        });
      }
  ...
  await appendMentorRows(sheets, 'MentorPlans', [{
    PlanId: planId,
    Email: email,
    UserId: userId,
    Version: generatedPlan.version || MENTOR_CACHE_VERSION,
    Status: 'active',
    ExamSnapshot: profile.examTarget || '',
    DaysLeftSnapshot: profile.daysLeftRange || '',
    PaceSnapshot: profile.pace || '',
    ActiveDayNumber: generatedPlan.dayNumber || 1,
    ProgressPercent: 0,
    LastGeneratedFromStateVersion: MENTOR_CACHE_VERSION,
    LastRefreshAt: now,
    CreatedAt: now,
    UpdatedAt: now,
  }]);
  await appendMentorRows(sheets, 'MentorTasks', (generatedPlan.tasks || []).map((task, index) => ({
    TaskId: task.taskId,
    PlanId: planId,
    Email: email,
    UserId: userId,
    DayNumber: generatedPlan.dayNumber || 1,
    SequenceNumber: index + 1,
    Type: task.taskType,
    Subject: task.subject || task.subjectName || '',
    Topic: task.topic || '',
    Title: task.subjectName === 'Repeated Mistakes' ? 'Repeated Mistakes' : (task.displayName || task.subjectName || 'Mentor Task'),
    MentorMessage: task.mentorMessage || '',
    Reason: task.reason || 'refresh',
    WhyThisText: task.whyThisText || '',
    PrimaryAction: task.ctaLabel || '',
    SecondaryAction: task.secondaryAction || 'Maybe later',
    Status: task.status || 'active',
    IsRequiredForUnlock: task.isRequiredForUnlock === false ? 'FALSE' : 'TRUE',
    DependsOnTaskIds: (task.dependsOnTaskIds || []).join(','),
    Source: task.source || 'refresh',
    SourceReference: task.sourceReference || '',
    SnoozeCount: task.snoozeCount || 0,
    SnoozedUntil: task.snoozedUntil || '',
    CreatedAt: now,
    ActivatedAt: task.status === 'active' ? now : '',
    CompletedAt: '',
    UpdatedAt: now,
  })));
  await updateMentorProfilePlanState(sheets, email, {
    MentorPlanId: planId,
    ActiveDayNumber: generatedPlan.dayNumber || 1,
    ProgressPercent: 0,
    LastPlanRefreshAt: now,
    PlanNeedsRebuild: 'FALSE',
    MentorLastSyncAt: now,
    MentorCacheVersion: MENTOR_CACHE_VERSION,
  });
  return getActiveMentorPlan(sheets, email);
}

async function updateMentorTaskStatus(sheets, email, taskId, updates = {}) {
  const { rows, headerIndex } = await readHeaderSheet(sheets, 'MentorTasks');
  const rowIndex = rows.findIndex(row => row[headerIndex.Email] === email && row[headerIndex.TaskId] === taskId);
  if (rowIndex === -1) throw new Error('Mentor task not found');
  const row = rows[rowIndex] || [];
  const nextUpdates = { ...updates };
  if (updates.SnoozeCount === '__increment__') {
    const nextSnoozeCount = Number(row[headerIndex.SnoozeCount] || 0) + 1;
    nextUpdates.SnoozeCount = String(nextSnoozeCount);
    if (nextSnoozeCount >= 2 && Number(row[headerIndex.QuestionCount] || 0) > 10) {
      nextUpdates.QuestionCount = '10';
      nextUpdates.WhyThisText = 'Is topic ko chhote step mein tod dete hain.';
    }
    if (nextSnoozeCount >= 3) {
      nextUpdates.Status = 'active';
      nextUpdates.Type = 'feedback_task';
      nextUpdates.PrimaryAction = 'Answer Now';
      nextUpdates.SecondaryAction = 'Later';
      nextUpdates.Reason = 'snooze_blocker';
      nextUpdates.MentorMessage = 'Is topic mein main blocker kya hai? Ek quick response se next step better decide hoga.';
    }
  }
  await updateMentorRow(sheets, 'MentorTasks', rowIndex + 2, {
    ...nextUpdates,
    UpdatedAt: new Date().toISOString(),
  });
}

async function appendMentorTaskLog(sheets, log = {}) {
  const now = new Date().toISOString();
  await appendMentorRows(sheets, 'MentorTaskLogs', [{
    LogId: log.logId || `MTL_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`,
    TaskId: log.taskId || '',
    PlanId: log.planId || '',
    Email: log.email || '',
    UserId: log.userId || '',
    ActionType: log.actionType || '',
    ActionValue: typeof log.actionValue === 'string' ? log.actionValue : JSON.stringify(log.actionValue || {}),
    CreatedAt: log.createdAt || now,
    SourcePage: log.sourcePage || '',
    QuizSessionId: log.quizSessionId || '',
    Notes: log.notes || '',
  }]);
}

module.exports.getMasterTopics = getMasterTopics;
module.exports.getMasterSubjects = getMasterSubjects;
module.exports.getMentorProfile = getMentorProfile;
module.exports.upsertMentorProfile = upsertMentorProfile;
module.exports.appendTaskFeedback = appendTaskFeedback;
module.exports.getMentorProfileWithPlanState = getMentorProfileWithPlanState;
module.exports.updateMentorProfilePlanState = updateMentorProfilePlanState;
module.exports.getStudentTopicState = getStudentTopicState;
module.exports.upsertStudentTopicState = upsertStudentTopicState;
module.exports.getActiveMentorPlan = getActiveMentorPlan;
module.exports.getMentorTasksForPlan = getMentorTasksForPlan;
module.exports.createMentorPlanSnapshot = createMentorPlanSnapshot;
module.exports.updateMentorTaskStatus = updateMentorTaskStatus;
module.exports.appendMentorTaskLog = appendMentorTaskLog;
