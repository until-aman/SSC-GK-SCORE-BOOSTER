...
async function processDailyRollover({
  ...
  const events = [];
  const taskUpdates = []; // Phase 10C: persist-ready change set for the write executor
    ...
    if (canMoveToPending(task)) {
      ...
      if (transition.allowed) {
        ...
        taskUpdates.push({
          ...
          expected: { planId: activePlan.planId, rowVersion: Number(task.rowVersion || 1), status: statusOf(task) },
          updates: { status: TASK_STATUS.PENDING, pendingReason: nextTasks[i].pendingReason, movedToPendingAt: nextTasks[i].movedToPendingAt, snoozeCount: nextTasks[i].snoozeCount },
      ...
      if (transition.allowed) {
        ...
        taskUpdates.push({
          ...
          planId: activePlan.planId,
          expected: { planId: activePlan.planId, rowVersion: Number(task.rowVersion || 1), status: statusOf(task) },
          updates: { status: TASK_STATUS.SCHEDULED, nextEligibleAt: transition.nextTask.nextEligibleAt },
          event: transition.event,
        });
        rescheduledCount += 1;
      }
