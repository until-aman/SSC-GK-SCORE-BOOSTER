...
function listPendingTasks(tasks = [], activePlan = {}, opts = {}) {
  ...
  const seen = new Set();
  return tasks
    .filter(task => isCurrentPlanTask(task, planId))
    .filter(task => statusOf(task) === TASK_STATUS.PENDING)
    .filter(task => !QUICK_CHECK_TYPES.has(task.type || task.taskType))
    .filter(task => ![TASK_STATUS.COMPLETED, TASK_STATUS.CANCELLED, TASK_STATUS.EXPIRED].includes(statusOf(task)))
    .filter(task => {
