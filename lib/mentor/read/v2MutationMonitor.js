...
const { calculateCalendarDay, toLocalDateKey, DEFAULT_TIMEZONE } = require('../domain/planDay');

const DEFAULT_AFFECTED_PLAN_ID = 'MP_1780920810055';

// Mirror userScopeFromIdentity({email}) exactly (u_<sha256(email).slice(0,16)>) without
// importing the mutation service (keeps this read-only module dependency-light).
function scopeFromEmail(email) {
  return `u_${crypto.createHash('sha256').update(String(email || 'unknown')).digest('hex').slice(0, 16)}`;
}
...
async function auditV2Mutations(sheets, { affectedPlanId = DEFAULT_AFFECTED_PLAN_ID, allowedUserHashes = [], rolloverAllowedUserHashes = [], rolloverFlagOn = false, nowIso = new Date().toISOString() } = {}) {
  ...
  // Phase 10F2: rollover LAG — eligible allowlisted active plans that are OWED a rollover
  // (calendarDay > LastProcessedCalendarDay) but have no completed ROLLOVER row for the day.
  // Only computed when the flag is on AND a rollover allowlist is supplied → no lag counted
  // when MENTOR_DAILY_ROLLOVER_V2 is off, and never for non-allowlisted users. WARNING-only.
  const rolloverAllow = new Set(rolloverAllowedUserHashes || []);
  const laggingPlans = [];
  if (rolloverFlagOn && rolloverAllow.size) {
    const completedRolloverKeys = new Set(rolloverMrRows.filter(r => String(mrGet(r, 'Status')).toLowerCase() === 'completed').map(r => mrGet(r, 'IdempotencyKey')));
    // newest active plan row per planId
    const activeRowByPlan = {};
    plans.rows.forEach(r => {
      if (String(pGet(r, 'Status')).toLowerCase() !== 'active') return;
      const pid = pGet(r, 'PlanId'); if (!pid) return;
      const ms = planTs(pGet(r, 'CreatedAt'));
      if (!activeRowByPlan[pid] || ms >= activeRowByPlan[pid].ms) activeRowByPlan[pid] = { r, ms };
    });
    Object.values(activeRowByPlan).forEach(({ r }) => {
      const scope = scopeFromEmail(pGet(r, 'Email'));
      if (!rolloverAllow.has(scope)) return;                            // eligible cohort only
      const planStart = pGet(r, 'PlanStartLocalDate');
      const tpd = Number(pGet(r, 'TotalPlanDays')) || 0;
      if (!planStart || !tpd) return;                                  // unknown day -> don't guess (no false lag)
      const tz = pGet(r, 'Timezone') || DEFAULT_TIMEZONE;
      const calDay = calculateCalendarDay({ planStartLocalDate: planStart, currentLocalDate: toLocalDateKey(nowIso, tz), totalPlanDays: tpd });
      const lpcd = Number(pGet(r, 'LastProcessedCalendarDay') || 0) || Number(pGet(r, 'ActiveDayNumber') || 0) || 1;
      if (calDay > lpcd) {
        const pid = pGet(r, 'PlanId');
        const key = `${ROLLOVER_KEY_PREFIX}${scope}:${pid}:${calDay}`;
        if (!completedRolloverKeys.has(key)) laggingPlans.push({ planId: pid, userScope: scope, calendarDay: calDay }); // masked: scope hash, never email
      }
    });
  }
  const rolloverEligiblePlansLagging = laggingPlans.length;

  return {
    rolloverEligiblePlansLagging,
    laggingPlansSample: laggingPlans.slice(0, 10),
...
function evaluateMonitorAlerts(audit = {}, { flags = {} } = {}) {
  ...
  // Phase 10F2: eligible plans owed a rollover the background path hasn't completed yet.
  // WARNING-only (a transient lag during background processing is expected); never CRITICAL.
  if (num(audit.rolloverEligiblePlansLagging) > 0) {
    add('WARNING', 'ROLLOVER_ELIGIBLE_PLANS_LAGGING', `${audit.rolloverEligiblePlansLagging} eligible plan(s) owed a rollover with no completed ROLLOVER row for the day`);
  }
