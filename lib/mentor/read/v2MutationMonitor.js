...
async function auditV2Mutations(sheets, { affectedPlanId = DEFAULT_AFFECTED_PLAN_ID, allowedUserHashes = [] } = {}) {
  ...
  // Active tasks per plan; only rollover-processed plans are judged against the cap.
  const activeByPlan = {};
  tasks.rows.filter(r => String(tGet(r, 'Status')).toLowerCase() === 'active' && planIdsWithRollover.has(tGet(r, 'PlanId'))).forEach(r => { const p = tGet(r, 'PlanId'); if (p) activeByPlan[p] = (activeByPlan[p] || 0) + 1; });
  ...
  // Plans that produced rollover events but have a blank LastProcessedCalendarDay.
  const planLastProcessed = {};
  plans.rows.forEach(r => { const p = pGet(r, 'PlanId'); if (p) planLastProcessed[p] = String(pGet(r, 'LastProcessedCalendarDay') || '').trim(); });
  const rolloverPlansMissingLastProcessedCalendarDay = [...planIdsWithRollover].filter(pid => !planLastProcessed[pid]).length;
