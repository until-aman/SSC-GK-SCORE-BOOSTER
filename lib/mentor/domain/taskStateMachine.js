...
function completeTask(task, context, now, source) {
  ...
  // Phase 9G1: quiz/practice tasks must complete with evidence — explicit quiz_sync
  // (or verified manual_recovery), never the default mentor_response, and quiz_sync
  // requires a non-empty linked quiz session id.
  const isQuizType = QUIZ_TYPES.has(task.type || task.taskType);
  if (isQuizType) {
    if (source !== COMPLETION_SOURCE.QUIZ_SYNC && source !== COMPLETION_SOURCE.MANUAL_RECOVERY) {
      return reject('INVALID_COMPLETION_SOURCE', 'Quiz tasks complete through quiz_sync (with evidence).', task);
    }
    if (source === COMPLETION_SOURCE.QUIZ_SYNC && !String(context.linkedQuizSessionId || '').trim()) {
      return reject('LINKED_QUIZ_SESSION_REQUIRED', 'quiz_sync completion requires a linked quiz session.', task);
    }
  }
  return applyCommon({
    ...task,
    ...
    linkedQuizSessionId: context.linkedQuizSessionId || task.linkedQuizSessionId || '',
    // completion clears any stale pending evidence
    pendingReason: '',
    movedToPendingAt: '',
  }, now);
}
...
function evaluateTaskTransition({ task, action, context = {}, now } = {}) {
  ...
  switch (action) {
    ...
    case TASK_ACTION.RESUME:
      ...
      // Resuming clears the pending evidence so the task is no longer canonical
      // pending (avoids a task looking pending while active due to stale fields).
      nextTask = applyCommon({ ...currentTask, status: context.resumeToInProgress ? TASK_STATUS.IN_PROGRESS : TASK_STATUS.ACTIVE, pendingReason: '', movedToPendingAt: '' }, timestamp);
