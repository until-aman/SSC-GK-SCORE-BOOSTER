...
function summarizeLegacy(legacy) {
  ...
  return {
    ...
    legacyStoredActiveDay: Number(legacy.plan?.activeDayNumber || legacy.activeDayNumber || 0) || null,
...
function summarizeAdapter(snapshot) {
  ...
  return {
    ...
    legacyStoredActiveDay: snapshot.activePlan ? snapshot.activePlan.activeDayNumber : null,
    canonicalCalendarDay: snapshot.calendarDay || null,
    canonicalActivePlanDay: snapshot.activePlanDay || null,
    planStartSource: snapshot.planStartSource || null,
    totalPlanDays: snapshot.totalPlanDays || null,
    ...
    diagnosticCodes: [...new Set((snapshot.diagnostics || []).map(d => d.code))],
  };
}
