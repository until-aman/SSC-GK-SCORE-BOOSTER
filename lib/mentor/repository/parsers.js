...
function parseProfile(rawSheet, email, opts = {}) {
  ...
  const profile = {
    ...
    customDaysLeft: str(get('CustomDaysLeft')),
    dailyGKTime: str(get('DailyGKTime')),
    pace: str(get('Pace')),
    timezone: str(get('Timezone')),
    planStartLocalDate: str(get('PlanStartLocalDate')),
...
function parsePlans(rawSheet, email) {
  ...
  rows.forEach(r => {
    ...
    plans.push({
      ...
      timezone: str(cell(r, map, 'Timezone')),
      planStartLocalDate: str(cell(r, map, 'PlanStartLocalDate')),
      totalPlanDays: num(cell(r, map, 'TotalPlanDays'), 0),
      unlockedDay: num(cell(r, map, 'UnlockedDay'), 0),
...
function parseTasks(rawSheet, email, planId) {
  ...
  rows.forEach(r => {
    ...
    tasks.push({
      ...
      // V2 pending-evidence fields (Phase 9C) — distinguish canonical pending
      // from legacy snoozed. Blank for legacy rows.
      pendingReason: str(cell(r, map, 'PendingReason')),
      movedToPendingAt: str(cell(r, map, 'MovedToPendingAt')),
      rowVersion: num(cell(r, map, 'RowVersion'), 0),
    });
  });
