...
const { isCanonicalPendingTask } = require('../services/dailyRolloverService');
...
function buildSnapshotFromRawData(rawData, userIdentity) {
  ...
  if (planRows.length) {
    ...
    // Phase 9C: canonical pending = current-generation tasks that are V2 pending
    // (Status=pending WITH PendingReason/MovedToPendingAt evidence). Legacy snoozed
    // rows (no v2 evidence) and quick-checks are excluded by isCanonicalPendingTask.
    snap.canonicalPendingTasks = snap.currentTasks.filter(isCanonicalPendingTask);
