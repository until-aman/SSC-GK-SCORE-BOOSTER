...
// Phase 10D-FIX-2: classify TRANSIENT Google Sheets errors that are safe to retry.
// Rate limits (429) and 5xx are server-side/transient; common network failures too.
// Deliberately NOT retriable: 4xx other than 429 (bad request/auth), and the
// repository's own optimistic-lock signals (STALE_*, TASK_NOT_FOUND, DUPLICATE_TASK_ROWS) —
// those are deterministic and must surface to the caller, never be retried.
const RETRIABLE_HTTP = new Set([429, 500, 502, 503, 504]);
const RETRIABLE_NET = /ECONNRESET|ETIMEDOUT|ECONNREFUSED|EAI_AGAIN|ENOTFOUND|EPIPE|socket hang up|network socket disconnected|request to .* failed/i;
const NON_RETRIABLE_DOMAIN = /STALE_|TASK_NOT_FOUND|DUPLICATE_TASK_ROWS|PLAN_ROW_/;

function isRetriableSheetsError(err) {
  if (!err) return false;
  const msg = String(err.message || err);
  if (NON_RETRIABLE_DOMAIN.test(msg)) return false;             // deterministic domain errors never retry
  const status = Number(err.code) || Number(err.status) || Number(err.response && err.response.status) || 0;
  if (RETRIABLE_HTTP.has(status)) return true;
  if (err.code && RETRIABLE_NET.test(String(err.code))) return true;
  return RETRIABLE_NET.test(msg);
}

// Bounded exponential backoff + jitter. `sleep` is injectable for fast tests.
async function withRetry(fn, { attempts = 4, baseMs = 250, maxMs = 2000, sleep } = {}) {
  ...
  for (let i = 0; i < attempts; i += 1) {
    try { return await fn(); }
      ...
      lastErr = e;
      if (i === attempts - 1 || !isRetriableSheetsError(e)) throw e;
      const backoff = Math.min(maxMs, baseMs * 2 ** i);
      await wait(backoff + Math.floor(Math.random() * baseMs)); // jitter
    }
  }
  throw lastErr;
}

function createSheetsIo(sheets, retryOpts) {
  const SID = process.env.GOOGLE_SHEET_ID;
  return {
    async read(tab) {
      const res = await withRetry(() => sheets.spreadsheets.values.get({ spreadsheetId: SID, range: `${tab}!A:ZZ` }), retryOpts);
      const v = (res.data && res.data.values) || [];
      return { headers: v[0] || [], rows: v.slice(1) };
    },
    async updateRow(tab, sheetRowNumber, rowValues) {
      await withRetry(() => sheets.spreadsheets.values.update({ spreadsheetId: SID, range: `${tab}!A${sheetRowNumber}`, valueInputOption: 'RAW', requestBody: { values: [rowValues] } }), retryOpts);
    },
    async appendRow(tab, rowValues) {
      await withRetry(() => sheets.spreadsheets.values.append({ spreadsheetId: SID, range: `${tab}!A:ZZ`, valueInputOption: 'RAW', insertDataOption: 'INSERT_ROWS', requestBody: { values: [rowValues] } }), retryOpts);
    },
  };
}
...
module.exports = { createSheetsMutationRepository, createSheetsIdempotencyStore, createSheetsPlanWriter, TASK_UPDATE_COLUMNS, isRetriableSheetsError, withRetry };
