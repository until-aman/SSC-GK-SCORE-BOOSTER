...
// Phase 10F2 — background daily-rollover execution toggle. When exactly "true", an
// ELIGIBLE rollover write is registered with Vercel waitUntil (non-blocking) instead of
// being awaited before the plan response. Fail-closed: only the exact string "true".
// This flag NEVER widens eligibility — the master gate (MENTOR_DAILY_ROLLOVER_V2) and the
// rollover allowlist still decide WHO; this only decides AWAITED vs BACKGROUND for them.
function isMentorDailyRolloverBackgroundEnabled() {
  return String(process.env.MENTOR_DAILY_ROLLOVER_BACKGROUND || '') === 'true';
}
...
module.exports = {
  ...
  isMentorDailyRolloverBackgroundEnabled,
};
