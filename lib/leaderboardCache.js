...
export function normalizeLeader(leader, index = 0) {
  return {
    ...
    coins: Math.round(Number(leader?.coins ?? leader?.totalScore ?? 0)),
...
export function toDisplayLeader(entry) {
  return {
    rank: entry?.rank,
    name: entry?.name,
    totalScore: Number(entry?.coins || 0),
    image: entry?.avatar || '',
...
export function buildLeaderboardCache({ leaders = [], currentUser = null, source = 'api', now = Date.now() }) {
  ...
  const top3Floor = top10[2]?.coins || 0;

  return {
    weekKey: getWeekKey(new Date(now)),
    top10,
    userRank: user ? { ...user, gapToTop3: Math.max(0, top3Floor - user.coins) } : null,
    lastFetchedAt: now,
