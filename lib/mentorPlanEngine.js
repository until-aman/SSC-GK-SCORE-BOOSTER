...
export function generateTodaysPlan(profile = {}, subjectHistory = [], mistakesPreview = {}, masterTopics = {}, options = {}) {
  ...
  if (repeatedMistakesCount > 0 && tasks.length < maxTasks) {
    ...
    tasks.push(makeTask({
      ...
      ctaLabel: 'Practice Mistakes',
      completionAction: 'Task is done',
  ...
  const finalTasks = applyAntiDuplication(tasks).slice(0, maxTasks).map((task, index) => ({
    ...task,
    // Permanent plan-order number — stays stable as tasks complete/snooze.
    taskNumber: index + 1,
...
function applyAntiDuplication(tasks) {
  ...
  return tasks.filter(task => {
    // Normalize subject/topic so the same topic from different sources
    // (e.g. "Q_PYQ_Polity" vs "Polity") collapses to one card. Same taskType +
    // same topic = a true duplicate; different types on one topic are kept.
    const subject = String(task.subject || '').replace(/^Q_PYQ_/, '').trim().toLowerCase();
    const topic = String(task.topic || task.displayName || task.reason || '').trim().toLowerCase();
    const key = `${task.taskType || ''}|||${subject || task.taskType}|||${topic}`;
